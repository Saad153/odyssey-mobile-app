...
function Example() {
  ...
  return(
  <Box _dark={{ bg: 'coolGray.800' }} _light={{ bg: 'white' }} flex="1" safeAreaTop w="100%">
    ...
    return(
      <Box>
        <Pressable onPress={() => console.log('You touched me')} 
          ...
          _light={{ bg: 'white', borderColor:'silver', borderWidth:1 }} 
          paddingRight={2}
        >
          <Box py="2">
            <HStack space={1}>
              <Avatar size="48px" source={require('../assets/jobicons/sea.jpg')} style={{backgroundColor:'silver'}} />
              <VStack>
  ...
  const renderHiddenItem = (data, rowMap) => {
    ...
    <HStack flex="1" pl="2">
      <Pressable w="70" ml="auto" cursor="pointer" bg="coolGray.200" justifyContent="center" 
        onPress={() => closeRow(rowMap, data.item.key)} _pressed={{ opacity: 0.5 }}
      >
      ...
      <Pressable w="70" cursor="pointer" bg="red.500" justifyContent="center" 
        onPress={() => deleteRow(rowMap, data.item.key)} _pressed={{ opacity: 0.5 }}
      >
        <VStack alignItems="center" space={2}>
